...
//([\\w\\/]+)int axisID;// numero dellâ€™asse (selezionabile da dip switch
su modulo Technosoft
...
ActuatorTechnoSoft::~ActuatorTechnoSoft(){
     ...
     for (std::map<int,TechnoSoftLowDriver *> ::iterator
it=motors.begin(); it!=motors.end(); ++it){
         deinit(it->first);
...
%d",it->first);
     ...
     DPRINT("Verifichiamo ora la dimensione della mappa statica: %d",
motors.size());
...
// La nuova funzione init si dovra' occupare della sola inizializzazione
del canale
// quindi la stringa dovra' contenere informazioni necessarie per la
sola eventuale
// apertura del canale
int ActuatorTechnoSoft::init(void*initialization_string){

     if(initChannelAlreadyDone){
         DPRINT("This object has already a communication channel
correctly initialized");
     ...
     if(regex_match(params, match, driver_match1, boost::match_extra)){
         ...
         DPRINT("String is matched: hostID: %d, btType: %d, baudrate:
%d,serial channel %s",hostID ,btType ,baudrate,dev_name.c_str());

         //SerialCommChannelTechnosoft objChannel(hostID, dev_name,
btType, baudrate);
         channel = new (std::nothrow) SerialCommChannelTechnosoft(hostID,
dev_name, btType, baudrate);
         if(channel==NULL){
             return -2;
         }
...
int ActuatorTechnoSoft::configAxis(void*initialization_string){
     ...
     if(regex_match(params, match, driver_match2, boost::match_extra)){
         ...
         std::map<int,TechnoSoftLowDriver* >::iterator i =
motors.find(axid); // iteratore alla mappa statica

         if(i==motors.end()){ // Il motore con il corrente axis id non e'
stato configurato.
                              // In questo caso creero' un nuovo oggetto
motore.
             TechnoSoftLowDriver* driver = new (std::nothrow)
TechnoSoftLowDriver();
             if(driver==NULL){
                 return -1;
             }
             ...
             if((val=driver->init(conf_path,axid))<0){
                 ERR("****************Iipologia di errore in fase di
inizializzazione dell'oggetto technosoft low driver %d",val);
                 delete driver;
                 driver = NULL;
                 return -2;
             }
...
motors.insert(std::pair<int,TechnoSoftLowDriver*>(axid,driver));
             DPRINT("Dimensione mappa statica alla fine della
configurazione dell'axisID %d avvenuta correttamente:
%d",axid,motors.size());
             return 0;
         }
         DPRINT("Axis id %d Ã¨ stato giÃ  configurato correttamente.",
axid);
         return -3;
...
//**********************************************************************
         //Le seguenti tre variabili sono relative a ciascun oggetto
TechnoSoftLowDriver
...
ActuatorTechnoSoft::ActuatorTechnoSoft(const ActuatorTechnoSoft&
objActuator){ // OVERLOADING COSTRUTTORE DI COPIA
...
//    internalHomingStateDefault =
objActuator.internalHomingStateDefault;
//    internalHomingStateHoming2 =
objActuator.internalHomingStateHoming2;

     // GESTIONE OGGETTO SerialCommChannelTechnosoft
     channel = new SerialCommChannelTechnosoft(hostID, dev_name, btType,
baudrate);
...
ActuatorTechnoSoft& ActuatorTechnoSoft::operator=(const
ActuatorTechnoSoft& objActuator){ // Overloading operatori di
assegnamento
     ...
     if(channel!=NULL){ //Prima distruggiamo il vecchio oggetto tec
technosoftLowdriver, per evitare un
                       // un errore di memori leak. Infatti la copia
viene eseguita su un oggetto gia' ESISTENTE!
         delete channel;
         channel = new SerialCommChannelTechnosoft(hostID, dev_name,
btType, baudrate);
     }

     // copia valori membri oggetto TechnoSoftLowDriver a destra
dell'uguale
     channel->pszDevName = (objActuator.channel)->pszDevName;
     channel->btType = (objActuator.channel)->btType;
     channel->baudrate = (objActuator.channel)->baudrate;
...
motors.find(axisID);
     ...
     if(i==motors.end()){
         // In questo caso il motore axisID non e' stato configurato, non
c'e' quindi alcun motore da inizializzare
         return -1;
     }
...
//        return -3;
     ...
     if(((i->second)->actuatorIDInMotion)==true)
         // ............... GESTIRE LO STOP VIRTUALE DEL MOTORE
ATTRAVERSO IL LOCK ...............
...
//        return -4;
     ...
     DPRINT("Object technosoftlowdriver with axisID = %d is
deinitialized",axisID);
     // Controllo lista vuota. Se e' vuota bisogna chiudere il
canale!!!!!
     return 0;
}
...
int ActuatorTechnoSoft::setParameter(int axisID,std::string
parName,std::string valueOfparName){
...
int ActuatorTechnoSoft::moveRelativeMillimeters(int axisID,double
deltaMillimeters){
...
motors.find(axisID);
     ...
     //double deltaMicroSteps =
round((STEPS_PER_ROUNDS_DEFAULT*N_ROUNDS_DEFAULT*CONST_MULT_TECHNOFT_DEFAULT*deltaMillimeters)/LINEAR_MOVEMENT_PER_N_ROUNDS_DEFAULT);
     double
deltaMicroSteps=(i->second)->getdeltaMicroSteps(deltaMillimeters);
     DPRINT("deltaMicroSteps = %f mm",deltaMicroSteps);
     if(deltaMicroSteps<=LONG_MIN || deltaMicroSteps>=LONG_MAX){ // solo
per adesso e necessario questo filtro..
         return -2;
     }
...
int ActuatorTechnoSoft::moveAbsoluteMillimeters(int axisID,double
millimeters){
...
motors.find(axisID);
     ...
     //double nMicroSteps =
round((N_ROUNDS_DEFAULT*STEPS_PER_ROUNDS_DEFAULT*CONST_MULT_TECHNOFT_DEFAULT*millimeters)/LINEAR_MOVEMENT_PER_N_ROUNDS_DEFAULT);
     double nMicroSteps = (i->second)->getdeltaMicroSteps(millimeters);
     DPRINT("nMicroSteps=%f\n",nMicroSteps);

     if(nMicroSteps<=LONG_MIN || nMicroSteps>=LONG_MAX){ // solo per
adesso e necessario questo filtro..
         return -2;
     }
     if((i->second)->selectAxis()<0){
         return -3;
...
//int ActuatorTechnoSoft::moveAbsoluteMillimetersHoming(double
millimeters){
//
//
//    // Calcolo argomento funzione moveAbsoluteSteps
//    double nMicroSteps =
round((STEPS_PER_ROUNDS_DEFAULT*N_ROUNDS_DEFAULT*CONST_MULT_TECHNOFT_DEFAULT*millimeters)/LINEAR_MOVEMENT_PER_N_ROUNDS_DEFAULT);
...
motors.find(axisID);
     ...
     if(mode==READ_COUNTER){ // Lettura posizione per mezzo del counter
(TPOS register)
         //long tposition;
         ...
         //std::cout<< "Il valore del counter e':"<<tposition
<<std::endl;
         //*deltaPosition_mm =
(tposition*LINEAR_MOVEMENT_PER_N_ROUNDS_DEFAULT)/(STEPS_PER_ROUNDS_DEFAULT*CONST_MULT_TECHNOFT_DEFAULT*N_ROUNDS_DEFAULT);
     }
     else if(mode==READ_ENCODER){ // Lettura posizione per mezzo
dell'encoder (Apos register)
         //long aposition;
         if((i->second)->getEncoder(deltaPosition_mm)<0){
             return -4;
         }
         //std::cout<< "Il valore dell'encoder e':"<<aposition
<<std::endl;
         //*deltaPosition_mm =
(aposition*LINEAR_MOVEMENT_PER_N_ROUNDS_DEFAULT)/(N_ENCODER_LINES_DEFAULT*N_ROUNDS_DEFAULT);
     }
     return 0;
}
...
int ActuatorTechnoSoft::getState(int axisID,int* state, std::string&
descStr){

     // ******************** NOTA:  le funzioni getState e getAlarms
devono essere eseguite in modalita' SEQUENZIALE,
     // e non indipendentemente l'una dall'altra in thread differenti,
perche' accedono agli stessi dati *********************
...
motors.find(axisID);
     ...
     (i->second)->stateInfoRequest = true; // Comunico al motore che sto
richiedendo info riguardanti gli allarmi


     short indexReg = 4; // see constant REG_SRH in TML_lib.h
     (i->second)->regSRHrequest = true;
     if(((i->second)->getStatusOrErrorReg(indexReg, contentRegSRH,
descStr))<0){
...
//    if(((i->second)->getStatusOrErrorReg(indexReg, contentRegSRL,
descStr))<0){
//        ERR("Reading state error: %s",descStr.c_str());
//        return -4;
//    }
//    (i->second)->regSRLrequest = false;
     ...
     if((i->second)->readyState){ // readyState = true se la procedura di
inizializzazione Ã¨ andata a buon fine. Accendo il primo bit
     ...
     if(contentRegSRH & ((uint16_t)1<<14)){
         ...
         descStr+="Reference position in absolute electronic camming
mode. ";
     }
     if(contentRegSRH & ((uint16_t)1<<15)){ // **************** Per
adesso lasciamolo cosi il fault status *********************
...
//    }
     ...
     if((i->second)->internalHomingStateDefault>0 ||
(i->second)->internalHomingStateHoming2>0){
...
int ActuatorTechnoSoft::getAlarms(int axisID, uint64_t* alrm,
std::string& descStr){
...
motors.find(axisID);
     ...
     (i->second)->alarmsInfoRequest = true; // Comunico al motore che sto
richiedendo info riguardanti gli allarmi

     short indexRegMER = 5; // see constant REG_MER in TML_lib.h
     (i->second)->regMERrequest = true;
     if((i->second)->getStatusOrErrorReg(indexRegMER, contentRegMER,
descStr)<0){
     ...
     (i->second)->regSRHrequest = true;
     if((i->second)->getStatusOrErrorReg(indexRegSRH, contentRegSRH,
descStr)<0){
         DERR("Reading alarms error: %s",descStr.c_str());
         return -4;
     }
     ...
     for(uint16_t i=0; i<sizeof(uint16_t)*8; i++){
         if(contentRegMER & ((uint16_t)1<<i)){ // se il bit i-esimo di
REG_MER Ã¨ 1, i=0,1,...,15
             if(i==0){
                 stCode|=ACTUATOR_CANBUS_ERROR; // IMPORTANTE:
ACTUATOR_CANBUS_ERROR Ã¨ di tipo int (32 bit)
                 // Nell'operazione di OR logico, automaticamente il
contenuto
                 // a destra dell'uguale viene prima memorizzato in una
locazione
                 // a 64 bit di tipo unsigned cosicchÃ© si possa fare l'OR
logico
                 // bit a bit con la variabile a primo membro
                 // In corrispondenza di questo errore accendo il bit 0
di *alarm
                 //desc.assign("CAN bus error. ");
                 descStr=descStr+"CAN bus error. ";
             }
             else if(i==1){
                 stCode|=ACTUATOR_SHORT_CIRCUIT; // In corrispondenza di
questo errore accendo il bit 1 di *alarm
                 ...
                 descStr+= "Hall sensor missing / Resolver error / BiSS
error / Position wrap around error. ";
             }
//            else if(i==6){
...
//            }
     ...
     *alrm = stCode;
     (i->second)->alarmsInfoRequest = false; // Comunico al motore che ho
terminato di richiedere info riguardanti gli allarmi
...
motors.find(axisID);
  ...
  int ActuatorTechnoSoft::resetAlarms(int axisID,uint64_t mode){
      ...
      // In the fault status the power stage is disabled, the MER
register signals
      // the errors occurred and  bit 15 from the SRH is set to high to
signal the fault state
      // ************************** Operazione di selezione axisID
***************************
      std::map<int,TechnoSoftLowDriver* >::iterator i =
motors.find(axisID);
      ...
      switch(mode){
          case 0:
             if((i->second)->resetFault()<0){
                 err = -3;
                 // Note: the drive-motor will return to FAULT status
(SRH.15=1) if there are
...
int ActuatorTechnoSoft::sendDataset(std::string& dataset){
    dataset.clear();
    dataset="{\"attributes\":[";
    dataset+="{\"name\":\"speed\",\"description\":\"Max speed of
trapezoidal
profile\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.001\",\"max\":\"500.0\",\"default\":\"400.0\"},";
    dataset+="{\"name\":\"acceleration\",\"description\":\"Acceleration
of trapezoidal
profile\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.001\",\"max\":\"2.0\",\"default\":\"0.6\"},";
    dataset+="{\"name\":\"isadditive\",\"description\":\"Specifies how is
computed the position to
reach\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"0\",\"max\":\"1\",\"default\":\"0\"},";
    dataset+="{\"name\":\"movement\",\"description\":\"Defines the moment
when the motion is
started\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"-1\",\"max\":\"1\",\"default\":\"1\"},";
    dataset+="{\"name\":\"referenceBase\",\"description\":\"Specifies how
the motion reference is
computed\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"0\",\"max\":\"1\",\"default\":\"1\"},";
    dataset+="{\"name\":\"highspeedhoming\",\"description\":\"Max speed
of trapezoidal profile for homing
procedure\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.001\",\"max\":\"15.0\",\"default\":\"10.0\"},";
    dataset+="{\"name\":\"lowspeedhoming\",\"description\":\"Speed of
trapezoidal profile for homing procedure, for repositioning slit at LSN
switch\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.001\",\"max\":\"3.0\",\"default\":\"1.0\"},";

dataset+="{\"name\":\"accelerationhoming\",\"description\":\"Acceleration
of trapezoidal profile for homing
procedure\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.001\",\"max\":\"0.6\",\"default\":\"0.3\"},";
    dataset+="{\"name\":\"isadditivehoming\",\"description\":\"Specifies
how is computed the position to reach for homing
...
procedure\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"0\",\"max\":\"1\",\"default\":\"1\"},";
    dataset+="{\"name\":\"numencoderlines\",\"description\":\"Number of
encoder
lines\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"1\",\"max\":\"10000000\",\"default\":\"800\"},";
    dataset+="{\"name\":\"nummicrostepsperstep\",\"description\":\"Number
of micro steps per
step\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"1\",\"max\":\"10000000\",\"default\":\"256\"},";
    dataset+="{\"name\":\"stepsperround\",\"description\":\"Number of
steps to perfor a complete
round\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"1\",\"max\":\"10000000\",\"default\":\"200\"},";
    dataset+="{\"name\":\"fixednumberofrounds\",\"description\":\"Number
of rounds for which the linear displacement is
known\",\"datatype\":\"int32\",\"direction\":\"Input\",\"min\":\"1\",\"max\":\"10000000\",\"default\":\"20\"},";

dataset+="{\"name\":\"lineardisplacement[mm]\",\"description\":\"Linear
displacement [mm] performed by slit associated with fixednumberofrounds
rounds\",\"datatype\":\"double\",\"direction\":\"Input\",\"min\":\"0.000000001\",\"max\":\"10000000\",\"default\":\"1.5\"}";
    dataset+="]}";
